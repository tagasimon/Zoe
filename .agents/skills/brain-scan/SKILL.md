---
name: brain-scan
description: Scrapes all Codex sessions from the past 7 days, identifies recurring workflows, and recommends what to build as skills, MCP plugins, agents, or AGENTS.md entries. Run weekly to keep the second brain sharp.
...
Analyse Simon's recent Codex sessions and surface automation opportunities.
...
```bash
find ~/.Codex/projects/ -name "*.jsonl" -newer $(date -v-${DAYS}d +%Y-%m-%d 2>/dev/null || date -d "-${DAYS} days" +%Y-%m-%d) 2>/dev/null | sort
```

If the date flag fails on macOS, use:
```bash
find ~/.Codex/projects/ -name "*.jsonl" -mtime -${DAYS}
```

Note the project folders — each maps to a working directory (e.g. `-Users-kazoobasimon-Codex-Zoe` = `/Users/kazoobasimon/Codex/Zoe`).
...
for path in glob.glob(os.path.expanduser("~/.Codex/projects/**/*.jsonl"), recursive=True):
...
- **AGENTS.md** — always-on preference or factual context
...
Also update the Skills Backlog in `AGENTS.md` with any new skill ideas identified.
